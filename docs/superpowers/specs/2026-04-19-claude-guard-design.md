# claude-guard — 설계 문서 (HISTORICAL)

> **NOTE (2026-04-20):** 이 문서는 **v0.1 brainstorming spec** 그대로입니다.
> v2.0 코드는 다르게 진화했습니다. 특히 다음 항목은 **더 이상 유효하지 않습니다**:
> - `L3 Red-team simulator`는 scan layer가 아니라 opt-in MCP tool (`redteam_probe`) 입니다. `Layer = "l1" | "l2"`만 존재합니다.
> - `engines.trivy`, `redteam.allowed_targets`는 Config에서 제거되었습니다.
> - `PLUGIN_UNTRUSTED`는 별도 approval flow가 아니라 loader warning 이름일 뿐입니다.
>
> 최신 사실은 [`docs/SECURITY_MODEL.md`](../../SECURITY_MODEL.md) 와 `src/` 코드를 보세요. 이 파일은 기록 용도로만 유지합니다.

**날짜:** 2026-04-19
**상태:** 설계 확정, 구현 플랜 대기 (역사적 스냅샷)
