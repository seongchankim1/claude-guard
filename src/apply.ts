...
export async function applyFixes(
  ...
  // If the caller didn't pick a mode, fall back to `dry_run` when the project
  // configured `fix.dry_run_default: true`. Lets teams adopt on a cautious
  // default without every caller having to remember `mode: "dry_run"`.
  const mode =
    opts.mode ?? (cfg.fix.dry_run_default ? "dry_run" : "checked");
  ...
  if (hasGit && mode !== "dry_run") {
    ...
    if (!branches.all.includes(branch)) {
      ...
      // An older claude-guard/fix-<id> branch already exists (a prior run,
      // possibly failed halfway). Silently checking it out would splice the
      // old state into the new rollback diff. Refuse so the caller can
      // either delete the branch or pass force=true.
      throw new Error(
        `FIX_BRANCH_EXISTS: branch ${branch} already exists from a prior run — delete it (git branch -D ${branch}) or pass force=true to reuse it`
      );
    } else {
