...
import {
  latestScanId as latestScanIdShared,
  loadScanArtifact,
} from "../findings-io.js";
...
Usage:
  ...
  claude-guard rollback <scan_id> [path] [--force]
      Revert a prior apply_fixes using its saved unified-diff patch. Refuses on a dirty tree unless --force.
...
async function main(argv: string[]): Promise<number> {
  ...
  if (cmd === "scan") {
    ...
    if (jsonMode || !process.stdout.isTTY) {
      ...
      if (r.engine_warnings && r.engine_warnings.length > 0) {
        process.stdout.write(
          color.yellow(
            `  engine warnings: ${r.engine_warnings.length}\n`
          )
        );
        for (const w of r.engine_warnings) {
          process.stdout.write(color.dim(`    [${w.engine}] ${w.message}\n`));
        }
      }
    }
  ...
  if (cmd === "rollback") {
    ...
    const [idArg, pathArg] = positional;
    if (!idArg) {
      process.stderr.write(
        "Usage: claude-guard rollback <scan_id> [path] [--force]\n"
      );
      return 1;
    }
    const projectPath = resolve(pathArg ?? ".");
    const { rollback } = await import("../rollback.js");
    const result = rollback(projectPath, idArg, { force });
    if (result.ok) {
      process.stdout.write(
  ...
  if (cmd === "sarif") {
    ...
    const findings = await loadFindings(projectPath, sid);
    // Include plugin rules so plugin-authored findings don't lose their
    // tooling.driver.rules metadata in the exported SARIF.
...
// Shared with server.ts so both surfaces see plugin/engine warnings.
const latestScanId = latestScanIdShared;

async function loadFindings(project: string, sid: string): Promise<Finding[]> {
  const a = await loadScanArtifact(project, sid);
  return a.findings;
}
